...
if __name__ == "__main__":
    ...
    if args.fullPlot: # if we want to plot the full aggregate
        ...
        if not args.animate: # if we don't want to animate the plot
            ...
            ax.set_box_aspect([1, 1, 1])
            ax.set_aspect('equal')
            #plt.savefig(os.path.join(args.root, f"voxels_window{suffix}"))
            plt.show()
            ...
            # Save
            anim.save(os.path.join(args.root, f"voxels_window_anim{suffix}.mp4"), fps=30, extra_args=['-vcodec', 'libx264'])
