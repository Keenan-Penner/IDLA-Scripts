...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root",
        type=str,
        default="C:\\Users\\keena\\OneDrive\\Bureau\\Math\\Python\\Scripts IDLA\\data\\multi-source\\An-dagger\\3D\\",
    )
    parser.add_argument(
        "--sim_number",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="agg",
        help="name of the file containing the points of the aggregate"
    )
    ...
    if args.fullPlot: # if we want to plot the full aggregate
        ...
        ax.voxels(x, y, z, voxel_grid, facecolors=colors, edgecolor='k', shade = False)
        ...
        # Calculate the maximum range across the three axes
        max_range = max(x_max - x_min, y_max - y_min, z_max - z_min)

        # Set the x-axis to be centered around 0
        x_centered_min = -max_range / 2
        x_centered_max = max_range / 2

        # Set the limits for each axis (center x-axis around 0)
        ax.set_xlim(x_centered_min, x_centered_max)
        ax.set_ylim(y_min - 0.5, y_min - 0.5 + max_range)
        ax.set_zlim(z_min - 0.5, z_min - 0.5 + max_range)

        ax.set_box_aspect([1, 1, 1])
        #ax.set_aspect('equal')

        plt.savefig(os.path.join(args.root, args.folder, f"voxels"), dpi = 500)
        #plt.show()
        ...
        if not args.animate: # if we don't want to animate the plot
            ...
            ax.set_box_aspect([1, 1, 1])
            ax.set_aspect('equal')
            plt.savefig(os.path.join(args.root, args.folder, f"voxels_window{suffix}"), dpi = 500)
            plt.show()
